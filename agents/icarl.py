    ...
    def update_representation(self, train_loader):
        ...
        for ep in range(self.epoch):
            for i, train_data in enumerate(train_loader):
                ...
                all_cls_num = 2 #len(self.new_labels) + len(self.old_labels)
                #target_labels = utils.ohe_label(train_y_copy, all_cls_num, device=train_y_copy.device).float()
                target_labels = utils.ohe_label(train_y, all_cls_num, device=train_y.device).float()
                if self.prev_model is not None:
                    ...
                    mem_target_labels = utils.ohe_label(mem_y, all_cls_num, device=mem_y.device).float()
                    target_labels = torch.cat([target_labels, mem_target_labels])
                else:
                    batch_x = train_x
